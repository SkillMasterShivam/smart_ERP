...
export function SalesForm({ initialData, isEdit = false }: SalesFormProps) {
  ...
  useEffect(() => {
    if (activeCompany) {
      // Fetch Debtors (Customers)
      apiClient.get('/ledgers')
        .then(res => setCustomers(res.data.data.filter((l: any) => l.type === 'Sundry Debtors' || l.group?.name?.includes('Debtor') || l.group?.name?.includes('Customer'))))
        .catch(console.error);
  ...
  const { register, control, handleSubmit, watch, setValue, formState: { errors } } = useForm<SalesFormValues>({
  ...
  const handleItemSelect = (index: number, itemId: string) => {
    ...
    if (selectedItem) {
      ...
      setValue(`items.${index}.rate`, selectedItem.selling_price || 0);
  ...
  const onSubmit = async (data: SalesFormValues) => {
    ...
    try {
      if (isEdit && initialData?.id) {
        await apiClient.put(`/sales/${initialData.id}`, data);
        toast.success('Draft updated successfully');
      } else {
        await apiClient.post('/sales', data);
        toast.success('Sales voucher created as Draft');
      }
        ...
        <div className="md:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="reference_number">Customer PO No.</Label>
                        ...
                        <tr key={field.id} className="border-b">
                          <td className="p-2">
                            ...
                            >
                              ...
                              {inventoryItems.map(i => <option key={i.id} value={i.id}>{i.name} (Stock: {i.current_quantity})</option>)}
                            </select>
                            {errors.items?.[index]?.item_id && <p className="text-xs text-red-500 mt-1">{errors.items[index].item_id.message}</p>}
                          </td>
                          <td className="p-2">
                            <Input 
                              type="number" step="0.0001" disabled={isPosted} 
                              {...register(`items.${index}.quantity`, { valueAsNumber: true })} 
                              className={watchItems[index]?.quantity > currentStock ? 'border-red-500' : ''}
                            />
                            {watchItems[index]?.quantity > currentStock && (
            ...
            {!isPosted && (
              <CardFooter className="flex flex-col gap-2">
                ...
                <Button variant="outline" type="button" className="w-full" asChild>
                  <Link href="/transactions/sales">Cancel</Link>
                </Button>
              </CardFooter>
            )}
          </Card>
