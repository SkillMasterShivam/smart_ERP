...
export default function EditPurchasePage() {
  ...
  const handlePost = async () => {
    if (!window.confirm('Are you sure you want to Post this voucher? This will permanently update inventory.')) return;
    setIsProcessing(true);
    try {
      await apiClient.post(`/purchases/${id}/post`);
  ...
  const handleCancel = async () => {
    if (!window.confirm('Are you sure you want to Cancel this voucher? This will reverse the inventory update.')) return;
    setIsProcessing(true);
    try {
      await apiClient.post(`/purchases/${id}/cancel`);
      toast.success('Voucher Cancelled successfully. Inventory reversed.');
      fetchVoucher(); // Refresh data
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <PurchaseForm initialData={voucher} isEdit={isDraft} />
    </div>
  );
}
